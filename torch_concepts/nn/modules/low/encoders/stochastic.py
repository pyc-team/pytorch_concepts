...
class StochasticLatentToConcept(BaseEncoder):
    ...
    def _predict_sigma(self, x):
        ...
        c_triang_cov = torch.zeros((c_sigma.shape[0], self.out_concepts, self.out_concepts), device=c_sigma.device)
        rows, cols = torch.tril_indices(row=self.out_concepts, col=self.out_concepts, offset=0)
        diag_idx = rows == cols
        c_triang_cov[:, rows, cols] = c_sigma
        c_sigma_activated = F.softplus(c_sigma[:, diag_idx])
        c_triang_cov[:, range(self.out_concepts), range(self.out_concepts)] = (c_sigma_activated + self.eps)
        return c_triang_cov
