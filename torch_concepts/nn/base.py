...
class BaseConceptLayer(ABC, nn.Module):
    ...
    """
    def __init__(self, out_concept_dimensions: Dict[int, Union[int, List[str]]]):
        super().__init__()
        self.concept_names = validate_and_generate_concept_names(out_concept_dimensions)
...
class ConceptEncoder(BaseConceptLayer):
    """
    ConceptEncoder generates concept embeddings with shape (batch_size, concept_dim1, ..., concept_dimN).
    ...
    """
    def __init__(self, in_features: int, out_concept_dimensions: Dict[int, Union[int, List[str]]], reduce_dim: int = None):
        super().__init__(out_concept_dimensions)
        self.in_features = in_features
        self.reduce_dim = reduce_dim
        if reduce_dim is not None:
            self.output_size = 1
        self.encoder = nn.Linear(in_features, self.output_size)
    ...
    def forward(self, x: torch.Tensor) -> ConceptTensor:
        ...
        Returns:
            ConceptTensor: Concept embeddings with shape (batch_size, concept_dim1, ..., concept_dimN).
        """
        emb = self.encoder(x)
        concept_shape = tuple(len(self.concept_names[dim]) for dim in sorted(self.concept_names.keys()) if dim != 0)
...
class ProbabilisticConceptEncoder(BaseConceptLayer):
    ...
    Samples are concept embeddings with shape (batch_size, concept_dim1, ..., concept_dimN).
    ...
    def __init__(self, in_features: int, out_concept_dimensions: Dict[int, Union[int, List[str]]]):
        super().__init__(out_concept_dimensions)
        self.in_features = in_features
        self.concept_mean_predictor = nn.Linear(in_features, self.output_size)
        self.concept_var_predictor = nn.Linear(in_features, self.output_size)
    ...
    def forward(self, x: torch.Tensor) -> ConceptDistribution:
        ...
        Returns:
            ConceptTensor: Concept distribution whose samples are concept embeddings with shape (batch_size, concept_dim1, ..., concept_dimN).
        """
...
class ConceptMemory(nn.Module):
    ...
    The output objects are concept tensors with shape (memory_size, concept_dim1, ..., concept_dimN).
    ...
    Attributes:
        ...
        out_concept_dimensions (Dict[int, Union[int, List[str]]]): Concept dimensions.
    """
    def __init__(self, memory_size: int, emb_size: int, out_concept_dimensions: Dict[int, Union[int, List[str]]]):
        super().__init__()
        self.concept_names = validate_and_generate_concept_names(out_concept_dimensions)
        self.output_size = compute_output_size(self.concept_names)
