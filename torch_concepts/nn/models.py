...
class ConceptModel(ABC, L.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert len(task_names) > 1 or not isinstance(
        ...
        self._bce_loss = isinstance(y_loss_fn, nn.BCELoss) or isinstance(
            y_loss_fn, nn.BCEWithLogitsLoss
        )
    ...
    def step(self, batch, mode="train") -> torch.Tensor:
        ...
        if self._bce_loss and self._multi_class and y_true.squeeze().dim() == 1:
            y_true_loss = (
                F.one_hot(
                    y_true.long(),
                    self.n_tasks,
...
class DeepConceptReasoning(ConceptExplanationModel):
        ...
        if "y_loss_fn" in kwargs:
            if isinstance(kwargs["y_loss_fn"], nn.CrossEntropyLoss):
                if use_bce:
                    ...
                    kwargs["y_loss_fn"] = (
                        lambda input, target, **kwargs: torch.nn.functional.nll_loss(
                            torch.log(
                                input / (input.sum(dim=-1, keepdim=True) + 1e-8) + 1e-8
                            ),
                            target,
                            **kwargs,
                        )
    ...
    def get_local_explanations(self, x, multi_label=False, **kwargs):
        ...
        y_pred = CF.logic_rule_eval(c_weights, c_pred, semantic=self.semantic)[:, :, 0]
    ...
    def get_global_explanations(self, x=None, multi_label=False, **kwargs):
        assert x is not None, "DCR requires input x to compute global explanations"
...
class ConceptMemoryReasoning(ConceptExplanationModel):
    ...
    def forward(self, x, c_true=None, y_true=None, **kwargs):
        ...
        if y_true is not None:
            c_rec_per_classifier = self._conc_recon(concept_weights, c_true, y_true)
...
class LinearConceptEmbeddingModel(ConceptExplanationModel):
    ...
    def step(self, batch, mode="train") -> torch.Tensor:
        ...
        prog = mode == "val"
        self.log(f"{mode}_weight_loss", w_loss, on_epoch=True, prog_bar=prog)

        if self.use_bias:
            b_loss = self.bias_reg * self.__predicted_bias.norm(p=1)
            loss += b_loss
            self.log(f"{mode}_bias_loss", b_loss, on_epoch=True, prog_bar=prog)
    ...
    def get_local_explanations(self, x, multi_label=False, **kwargs):
        ...
        linear_equations = CF.linear_equation_expl(
            c_weights,
            y_bias,
            {
                1: self.concept_names,
        ...
        for i in range(x.shape[0]):
            ...
            for j in range(self.n_tasks):
                ...
                if predicted_task:
                    ...
                    sample_expl.update({self.task_names[j]: predicted_eq})
            local_expl.append(sample_expl)
    ...
    def get_global_explanations(self, x=None, **kwargs):
        assert x is not None, (
            "LinearConceptEmbeddingModel requires input x "
...
class LinearConceptMemoryReasoning(ConceptExplanationModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.equation_memory = torch.nn.Embedding(memory_size, latent_dim)
    ...
    def forward(self, x, c_true=None, **kwargs):
        ...
        equation_weights = self.equation_decoder(self.equation_memory.weight).unsqueeze(
            dim=0
        )
    ...
    def get_global_explanations(self, x=None, **kwargs):
        concept_weights = self.equation_decoder(self.equation_memory.weight).unsqueeze(
            dim=0
        )
...
class StochasticConceptBottleneckModel(ConceptModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.bottleneck = pyc_nn.StochasticConceptBottleneck(
            latent_dim,
            concept_names,
        ...
        self.y_predictor = nn.Sequential(
            torch.nn.Linear(len(concept_names), latent_dim),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(latent_dim, len(task_names)),
            torch.nn.Sigmoid(),
        )
    ...
    def step(self, batch, mode="train") -> torch.Tensor:
        ...
        x, c_true, y_true = batch
        y_pred, c_pred, c_pred_av, emb = self.forward(
            x, c_true=c_true, current_epoch=self.trainer.current_epoch
        )
        ...
        # Final loss
        total_loss = (
            self.concept_reg * concept_loss + task_loss + self.cov_reg * cov_loss
        )
        ...
        if c_pred_av is not None:
            ...
            c_avg_auc = roc_auc_score(
                c_true.cpu().view(-1), (c_pred_av.cpu().view(-1) > 0.5).float()
            )
    ...
    def forward(self, x, c_true=None, **kwargs):
        ...
        # Hard MC concepts
        temp = compute_temperature(kwargs["current_epoch"], self.num_epochs).to(
            c_pred.device
        )
