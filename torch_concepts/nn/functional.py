...
def _default_concept_names(shape: List[int]) -> Dict[int, List[str]]:
    ...
    for dim in range(len(shape)):
        concept_names[dim+1] = [f"concept_{dim+1}_{i}" for i in range(shape[dim])]
    return concept_names


def intervene(c_pred: torch.Tensor, c_true: torch.Tensor, indexes: torch.Tensor, concept_names: Dict[int, List[str]] = None) -> ConceptTensor:
    """
    ...
    Args:
        ...
        indexes (Tensor): Boolean ConceptTensor indicating which concepts to intervene on.
    ...
    if c_pred.shape != c_true.shape:
        raise ValueError("Predicted and true concepts must have the same shape.")
...
def concept_embedding_mixture(c_emb: torch.Tensor, c_scores: torch.Tensor, concept_names: Dict[int, List[str]] = None) -> ConceptTensor:
    """
    ...
    Args:
        c_emb (Tensor): Concept embeddings with shape (batch_size, n_concepts, emb_size).
...
def intervene_on_concept_graph(c_adj: torch.Tensor, indexes: List[Union[int, str]], concept_names: Dict[int, List[str]] = None) -> ConceptTensor:
    """
    ...
    if concept_names is None:
        ...
        for dim in range(len(c_adj.shape)):
            concept_names[dim] = [f"concept_{dim}_{i}" for i in range(c_adj.shape[dim])]
    ...
    if isinstance(indexes[0], str):
        indexes_from_str = [concept_names[1].index(name) for name in indexes if name in concept_names[1]]
...
def linear_memory_eval(concept_weights: torch.Tensor, c_pred: torch.Tensor) -> torch.Tensor:
    """
    ...
    Returns:
        Tensor: Predictions made by the linear models with shape (batch_size, n_classes, memory_size).
    """
    return torch.einsum('mcys,bc->bym', concept_weights, c_pred)


def logic_memory_eval(concept_weights: torch.Tensor, c_pred: torch.Tensor, memory_idxs: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def logic_memory_reconstruction(c_rec_per_classifier: torch.Tensor, c_true: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    """
...
def logic_memory_explanations(concept_logic_weights: torch.Tensor, concept_names: Dict[int, List[str]] = None) -> Dict[str, Dict[str, str]]:
    """
    ...
    """
    if len(concept_logic_weights.shape) != 4 or concept_logic_weights.shape[3] != 3:
        raise ValueError("The concept logic weights must have 4 dimensions (memory_size, n_concepts, n_tasks, 3).")
    ...
    for task_idx in range(n_tasks):
        for mem_idx in range(memory_size):
            rule = [("~ " if concept_roles[mem_idx, concept_idx, task_idx] == 1 else "") + c_names[concept_idx]
...
def selective_calibration(c_confidence: torch.Tensor, target_coverage: float, concept_names: Dict[int, List[str]] = None) -> ConceptTensor:
    """
...
def confidence_selection(c_confidence: torch.Tensor, theta: torch.Tensor, concept_names: Dict[int, List[str]] = None) -> ConceptTensor:
    """
