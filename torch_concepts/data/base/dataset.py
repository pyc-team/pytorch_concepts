...
class ConceptDataset(Dataset):
    ...
    def graph_native(self) -> Optional[ConceptGraph]:
        """Graph originally provided by the dataset, before generation."""
        return self._graph_native

    @property
    def graph_generator(self):
        """Graph generator created by :meth:`precompute_graph`, if any."""
    ...
    def precompute_graph(
        ...
        **kwargs,
        ...
        """Create, run, and retain a graph generator for this dataset.

        Selects and creates the fixed or learnable generator registered for
        ``source``. When ``use_as_gt=True``, the new generator is run and its
        result replaces :attr:`graph`; :attr:`graph_native` is never modified.
        When false, the generator is retained without being materialized.
        ...
        source : str
            Registered fixed or learnable graph-generator source.
        use_as_gt : bool, default True
            Store the generated graph in :attr:`graph`. If false, leave
            :attr:`graph` unchanged.
        **kwargs
            Additional arguments forwarded to the selected generator.
        """
        ...
        generator_classes = [
            ...
            if source in generator_cls._sources
        ]
        if len(generator_classes) != 1:
            available_sources = sorted(
                set(GraphGeneratorFixed._sources)
                | set(GraphGeneratorLearnable._sources)
            )
            raise ValueError(
                f"Source {source!r} must identify exactly one graph generator "
                f"family; available sources: {available_sources}."
            )

        generator_cls = generator_classes[0]
        self._graph_generator = generator_cls(
            name=name,
            source=source,
            **kwargs,
        )
    ...
    def _compute_graph(self) -> ConceptGraph:
        """Generate and return a graph from the complete dataset.

        Returns
        -------
        ConceptGraph
            Graph generated by the retained graph generator.
        """
        return self.graph_generator.generate(self)
