...
class AWA2DataModule(ConceptDataModule):
    ...
    split generated during the first build step.

    Parameters
    ----------
    root : str, optional
        Root directory where the AwA2 data is stored.
        Must contain ``JPEGImages/``, ``predicate-matrix-binary.txt``, and
        ``classes.txt``.  Default: ``None`` (auto-creates ``./data/AWA2``).
    image_size : int, optional
        Side length (pixels) used when loading raw images.  Default: 224.
    seed : int, optional
        Random seed for train / val / test split generation.  Default: 42.
    ...
    splitter : Splitter, optional
        Splitting strategy.  Default: ``NativeSplitter()`` (uses the split
        generated by the dataset build step).
