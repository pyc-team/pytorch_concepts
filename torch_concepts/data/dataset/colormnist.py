...
class ColorMNISTDataset(ConceptDataset):
    """Dataset class for the ColorMNIST dataset.

    This dataset represents a small expert system that models the relationship
    between color features and various attributes in the MNIST dataset.
    """
    ...
    def files_to_download_names(self) -> List[str]:
        ...
        considered present."""
        return {"data": "mnist_data.pt",
                "targets": "mnist_targets.pt"}
    ...
    def download(self):
        train_data = MNIST(root=self.root, train=True, download=True, transform=self.transform)
        test_data = MNIST(root=self.root, train=False, download=True, transform=self.transform)

        data = torch.cat([train_data.data, test_data.data], dim=0)
        targets = torch.cat([train_data.targets, test_data.targets], dim=0)

        torch.save(data, os.path.join(self.root_dir, "mnist_data.pt"))
        torch.save(targets, os.path.join(self.root_dir, "mnist_targets.pt"))

    def build(self):
        self.maybe_download()

        # load raw data
        data = torch.load(os.path.join(self.root_dir, "mnist_data.pt"))
        targets = torch.load(os.path.join(self.root_dir, "mnist_targets.pt"))
        ...
        concepts['number'] = targets.numpy()
        concepts['parity'] = (concepts['number'] % 2 == 0).astype(int)
        concepts['color'] = concepts_dict['colors'].numpy()

        # construct the graph
        graph = pd.DataFrame(0, index=concepts.columns, columns=concepts.columns)
        graph.loc['number', 'parity'] = 1
