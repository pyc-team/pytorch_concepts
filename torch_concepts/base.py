...
class ConceptTensor(torch.Tensor):
    ...
    Attributes:
        ...
        concept_names (Dict[int, List[str]]): Names of concepts for each dimension.
    """
    def __new__(cls, data: torch.Tensor, concept_names: Dict[int, List[str]] = None, *args, **kwargs) -> 'ConceptTensor':
    ...
    def _generate_default_concept_names(shape):
        ...
        for dim in range(len(shape)):
            concept_names[dim] = [f"concept_{dim}_{i}" for i in range(shape[dim])]
        return concept_names

    @staticmethod
    def _check_concept_names(tensor: torch.Tensor, concept_names: Dict[int, List[str]]) -> Dict[int, List[str]]:
        ...
        if not isinstance(concept_names, dict):
            raise ValueError("concept_names must be a dictionary with dimension indices as keys and lists of names as values.")

        # Check that the concept dictionary IDs are within the range of the tensor's dimensions
        for dim in concept_names.keys():
            if dim < 0 or dim >= len(tensor.shape):
                raise ValueError(f"Dimension {dim} is out of range for the tensor with shape {tensor.shape}.")
        ...
        for dim in range(len(tensor.shape)):
            if dim not in concept_names:
                concept_names[dim] = [f"concept_{dim}_{i}" for i in range(tensor.shape[dim])]
            elif len(concept_names[dim]) != tensor.shape[dim]:
                raise ValueError(f"Number of concept names for dimension {dim} must match the size of that dimension in the tensor.")
    ...
    @classmethod
    def concept(cls, tensor: torch.Tensor, concept_names: Dict[int, List[str]] = None) -> 'ConceptTensor':
        """
    ...
    def update_concept_names(self, new_concept_names: Dict[int, List[str]]):
        ...
        Attributes:
            new_concept_names: Dictionary with dimension indices as keys and lists of new concept names as values.
        """
        for dim, names in new_concept_names.items():
            if dim not in self.concept_names:
                raise ValueError(f"Dimension {dim} is not present in the current concept names.")
            if len(names) != len(self.concept_names[dim]):
                raise ValueError(
                    f"Number of new concept names for dimension {dim} must match the size of that dimension.")
            self.concept_names[dim] = names

    def extract_by_concept_names(self, target_concepts: Dict[int, List[Union[int, str]]]) -> 'ConceptTensor':
        """
        Extract a subset of concepts from the ConceptTensor.

        Attributes:
            target_concepts: Dictionary where keys are dimensions and values are lists of concept names or indices to extract.
        ...
        for dim, concepts in target_concepts.items():
            if dim not in self.concept_names:
                raise ValueError(f"Concept names are not set for dimension {dim}.")

            if isinstance(concepts[0], str):
                indices = [self.concept_names[dim].index(name) for name in concepts if name in self.concept_names[dim]]
                if len(indices) != len(concepts):
                    raise ValueError("Some concept names are not found in the tensor's concept names.")
            else:
                indices = concepts

            extracted_data = extracted_data.index_select(dim, torch.tensor(indices, device=self.device))
            new_concept_names[dim] = [self.concept_names[dim][i] for i in indices]
    ...
    def new_empty(self, *shape):
        ...
        new_concept_names = {dim: self.concept_names[dim][:shape[dim]] for dim in range(len(shape))}
    ...
    def _update_concept_names_for_new_shape(self, new_shape):
        ...
        for dim, size in enumerate(new_shape):
            if dim in self.concept_names and len(self.concept_names[dim]) == size:
                new_concept_names[dim] = self.concept_names[dim]
            else:
                new_concept_names[dim] = [f"concept_{dim}_{i}" for i in range(size)]
    ...
    def view(self, *shape):
        ...
        if hasattr(self, 'concept_names'):
            new_tensor.concept_names = self._update_concept_names_for_new_shape(new_tensor.shape)
        return new_tensor

    def reshape(self, *shape):
        """
        Reshape the tensor to the specified shape and update concept names accordingly.
        """
    ...
    def transpose(self, dim0, dim1):
        """
        Transpose two dimensions of the tensor and update concept names accordingly.
        """
        ...
        if dim0 in self.concept_names and dim1 in self.concept_names:
            new_concept_names[dim0], new_concept_names[dim1] = self.concept_names[dim1], self.concept_names[dim0]
    ...
    def permute(self, *dims):
        """
        Permute the dimensions of the tensor and update concept names accordingly.
        """
    ...
    def unsqueeze(self, dim):
        ...
        new_tensor = new_tensor.as_subclass(ConceptTensor)
        if hasattr(self, 'concept_names'):
            new_concept_names = {i + 1 if i >= dim else i: v for i, v in self.concept_names.items()}
    ...
    def __getitem__(self, key):
        ...
        if isinstance(sliced_tensor, torch.Tensor) and not isinstance(sliced_tensor, ConceptTensor):
        ...
        for dim, names in self.concept_names.items():
            if dim < len(sliced_tensor.shape):
                ...
                if isinstance(index, slice):
                    ...
                    new_concept_names[dim] = [names[i] for i in index if i < len(names)]
...
class ConceptDistribution(torch.distributions.Distribution):
    ...
    def __init__(self, base_dist: torch.distributions.Distribution, concept_names: Dict[int, List[str]] = None):
        self.base_dist = base_dist
        self.concept_names = ConceptTensor._check_concept_names(base_dist.mean, concept_names.copy())
        super().__init__()
    ...
    def rsample(self, sample_shape: torch.Size = torch.Size()) -> ConceptTensor:
        ...
        return ConceptTensor.concept(sample, concept_names=self.concept_names.copy())
