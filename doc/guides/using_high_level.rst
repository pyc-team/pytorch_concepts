...
Working at this level involves four building blocks:

- **Annotations** — describe the concepts and tasks (names, cardinalities, types).
...
:class:`~torch_concepts.Annotations` describe the structure of your concept space: the ordered
``labels``, their ``cardinalities`` (``1`` for binary, ``>1`` for categorical), their ``types``,
and optional per-label ``metadata``. They are the single source of truth that models, losses
and metrics use to route each concept correctly. Most datasets expose them directly as
``dataset.annotations``.

**Example.** Build annotations for two binary concepts and one categorical task:
...
.. code-block:: python
   ...
   annotations = Annotations(
       ...
       cardinalities=[1, 1, 2],
       metadata={
           'c1': {'type': 'binary', 'description': 'Concept 1'},
           'c2': {'type': 'binary', 'description': 'Concept 2'},
           'xor': {'type': 'categorical', 'description': 'XOR task'},
       },
   )

When you have many concepts of mixed types, :class:`~torch_concepts.GroupConfig` lets you
configure things **once per type** instead of per concept:
