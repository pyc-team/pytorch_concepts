...
from torch_concepts import Variable, Annotations, AxisAnnotation
from torch_concepts.distributions import Delta
from torch_concepts.nn import Factor, HyperLinearPredictor, ProbEncoderFromEmb, ExogEncoder, \
    ProbabilisticGraphicalModel, ForwardInference, ProbEncoderFromExog, RandomPolicy, DoIntervention, intervention
...
concept_preds_tensor = inference_engine.query(query_concepts, evidence=initial_input)
print(f"Query Concepts: {query_concepts} - Variable sizes: {[model.concept_to_variable[c].size for c in query_concepts]}")
print(concept_preds_tensor[:5])

c_annotations = Annotations({1: AxisAnnotation(["c1"])})
int_policy_c = RandomPolicy(out_annotations=c_annotations, scale=100, subset=["c1"])
int_strategy_c = DoIntervention(model=model.factor_modules, constants=-10)
