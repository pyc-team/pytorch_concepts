...
def main():
    ...
    (x_train, c_train, y_train,
     concept_names, class_names) = (data.data, data.concept_labels,
                                    data.target_labels, data.concept_attr_names,
                                    data.task_attr_names)

    # # (for testing CMR with two classes)
    ...
    concept_memory_decoder = torch.nn.Sequential(
        ...
        torch.nn.Unflatten(-1, (n_concepts, n_classes, memory_states)),
        Annotate([concept_names, class_names, memory_names], [1, 2, 3]),
    )
    model = torch.nn.Sequential(encoder, concept_bottleneck,
    ...
    for epoch in range(n_epochs):
        ...
        concept_weights = concept_memory_decoder(
            latent_concept_memory.weight).softmax(dim=-1).unsqueeze(dim=0)
