...
def main():
    ...
    ## specific for CEBaB ##
    concepts_type = 'continuous'  # 'discrete' or 'continuous'
    input_type = 'text'  # 'text' or 'image'
    
    # Model configuration
    backbone_name = 'bert-base-uncased'  # Options: 'bert-base-uncased', etc. 
    embedding_dims = 256           # Dimension of latent space after backbone
    ...
    print("   Using continuous concepts and task. Loss: MSELoss")
    concept_output_dims = len(concept_indices)
    task_dims = 1
    # specify loss functions for continuous concepts and task
    loss_fn = nn.MSELoss()
    ...
    for epoch in range(n_epochs):
        ...
        for batch in progress_bar:
            ...
            c_targets = c[:, concept_indices].float()
            y_targets = c[:, task_index:task_index+1].float()
        ...
        concept_mse = nn.MSELoss()(all_concept_preds, all_concept_targets)
        task_mse = nn.MSELoss()(all_task_preds, all_task_targets)
        print(f"   Concept Loss: {avg_concept_loss:.4f} | Concept MSE: {concept_mse.item():.4f}")
        print(f"   Task Loss: {avg_task_loss:.4f} | Task MSE: {task_mse.item():.4f}")
    ...
    print(f"  1. Loading CEBaB dataset using continuous concepts")
