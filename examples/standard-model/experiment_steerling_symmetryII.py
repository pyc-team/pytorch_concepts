...
def main():
    ...
    hidden = backbone(input_ids).float().retain_grad()
    output = inference.query(["new_token"], evidence={"input": hidden}, return_logits=True)
    print(f"Predicted logits for 'Yes': {output}")
    ...
    concept_model.zero_grad()
    prob_yes_rel.backward()
    input_gradients = hidden.grad
    ...
    h_bar_output = inference.query(["h_bar"], evidence={"input": hidden}, return_logits=True)
    print(f"h_bar output shape: {h_bar_output.logits.shape}")
    print(f"\nRetaining gradients for h_bar output logits...")
    h_bar_output.logits.retain_grad()
    h_bar_output.logits.backward()
    h_bar_gradients = h_bar_output.logits.grad
    ...
    print(f"SymmetryII metric: {symmetryII_metric}")
