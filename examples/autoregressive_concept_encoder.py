...
from torch_concepts.nn import ConceptScorer, AutoregressiveConceptEncoder
...
def main():
    ...
    c_sampler = AutoregressiveConceptEncoder(emb_size, n_concepts, 2*emb_size, concept_names)
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 100 == 0:
            print(f"Epoch {epoch}: Task Loss {task_loss.item():.2f}, Concept Loss {concept_loss.item():.2f}")
    ...
    print(f"Predicted concepts: {c_pred.describe()}")
