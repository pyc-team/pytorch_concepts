...
def main():
    ...
    c1_encoder = ParametricCPD(concepts[0], parametrization={'probs': layers['concept_encoder']}, parents=[embs[0]])
    c2_encoder = ParametricCPD(concepts[1], parametrization={'probs': deepcopy(layers['concept_encoder'])}, parents=[embs[1]])
    y_predictor = ParametricCPD(tasks, parametrization={'probs': layers['task_predictor']}, parents=[*concepts, *embs], aggregate=layers['aggr_embeddings'])
