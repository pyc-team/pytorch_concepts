...
def main():
    ...
    assert torch.allclose(obs_c1.probs["c2"], c[:, 1:2])
    # ...and clamping c1 propagates to xor.
    assert not torch.allclose(obs_c1.probs["xor"], base.probs["xor"])
    print("evidence: observing only c1 leaves c2 as model, moves xor         OK")

    # --- intervention / do: force a member, read the task's response -------
    do_hi = engine.query(["xor"], evidence={"x": x, "c1": ones}).probs["xor"]
    do_lo = engine.query(["xor"], evidence={"x": x, "c1": zeros}).probs["xor"]
    ...
    do_group = engine.query(["xor"], evidence={"x": x, "concepts": torch.cat([ones, ones], dim=1)}).probs["xor"]
    ...
    y1_c1 = [engine.query(["y1"], evidence={"x": x, "c1": v}).probs["y1"] for v in (ones, zeros)]
    y1_c2 = [engine.query(["y1"], evidence={"x": x, "c2": v}).probs["y1"] for v in (ones, zeros)]
