...
for step in range(EPOCHS):
    ...
    loss_c1 = loss_fn(out.probs['c1'], batch['c1'])
    loss_c2 = loss_fn(out.probs['c2'], batch['c2'])
    loss_y = loss_fn(out.probs['y'], batch['y'])
    ...
    if step % 1000 == 0:
        print(f"Step {step}: Loss {loss.item():.4f} | c1 acc {(out.probs['c1'] > 0.5).float().eq(batch['c1']).float().mean().item():.4f} | c2 acc {(out.probs['c2'] > 0.5).float().eq(batch['c2']).float().mean().item():.4f} | y acc {(out.probs['y'] > 0.5).float().eq(batch['y']).float().mean().item():.4f}")
...
y_pred = torch.where(out.probs['y'] > 0.5, 1.0, 0.0)
y_true = test_batch['y']
...
for step in range(EPOCHS):
    ...
    recon = F.binary_cross_entropy(out.probs['y'], batch['y'])
    kl_c1 = bernoulli_kl(out.guide_params['probs']['c1'], out.probs['c1'])
    kl_c2 = bernoulli_kl(out.guide_params['probs']['c2'], out.probs['c2'])
    kl = kl_c1 + kl_c2
    loss = recon + kl
    optim.zero_grad()
    loss.backward()
    ...
    if step % 1000 == 0:
        c1_acc = binary_accuracy(out.guide_params['probs']['c1'].detach(), batch['c1']).item()
        c2_acc = binary_accuracy(out.guide_params['probs']['c2'].detach(), batch['c2']).item()
        y_acc = binary_accuracy(out.probs['y'].detach(), batch['y']).item()
