"""
In this example, we demonstrate how to implement a Concept Bottleneck Model (CBM) using the `torch_concepts` library. 
Experiment settings:
- Dataset: XOR dataset with two concepts (c1, c2) and one task (xor)
- Model: A CBM with a latent embedding layer, concept encoder, and task predictor
- Inference engine: Deterministic inference

Expected outcome:
- The model should learn to predict the concepts while struggling to predict the task due 
the linear task predictor.
"""
